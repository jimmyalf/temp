(function($){
	window.FileManager = $.extend(BaseManager, {
		...
		init : function() {
			...
			$().bind('action:findconnections', function() {t.findConnection(t.focusedFile);});
			...
			$().bind('selection:changed', function() {
				$(['cut', 'copy', 'delete', 'zip', 'findconnections']).each(function(i, v) {
		...
		findConnection : function(f) {
			RPC.exec('fm.findConnections', {file : f.path }, function (data) {
				alert(data.result.data[0][0]);
			});
		},
