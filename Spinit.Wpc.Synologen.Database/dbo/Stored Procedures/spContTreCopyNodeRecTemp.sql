create PROCEDURE spContTreCopyNodeRecTemp
					@oldId INT,
					@parent INT,
					@order INT,
					@user NVARCHAR (100),
		...
		BEGIN
			...
			DECLARE get_tree CURSOR LOCAL FOR
				SELECT	cPgeId,
						...
						cUnPublishDate,
						cLockedBy
				FROM	dbWpcSynologen_old_dev.dbo.tblContTree
				WHERE	cId = @oldId
				...
				BEGIN
					...
					INSERT INTO tblContPage
						...
						SELECT	cPgeTpeId,
								...
								cCreatedBy,
								cCreatedDate
						FROM	dbWpcSynologen_old_dev.dbo.tblContPage
						WHERE	cId = @pgeId
			...
			DECLARE get_bseGrp CURSOR LOCAL FOR
				SELECT	cBseGrpId
				FROM	dbWpcSynologen_old_dev.dbo.tblContTreeBaseGroups
				WHERE	cTreId = @oldId
			...
			WHILE (@@FETCH_STATUS <> -1)
				BEGIN
					...
					EXECUTE spContTreCopyNodeRecTemp @childId, 
												 @newId, 
												 @orderCount,
												 @user,
												 NULL,
