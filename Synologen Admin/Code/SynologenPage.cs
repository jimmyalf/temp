...
namespace Spinit.Wpc.Synologen.Presentation.Code {
	...
	public class SynologenPage : System.Web.UI.Page {
		...
		private Data.SqlProvider _provider = null;
		...
		protected bool IsInRole(MemberRoles.Roles role) {
			...
			bool access = true;
			string memberComponentName = Member.Business.Globals.ComponentName;
			try {
		...
		protected bool IsInSynologenRole(SynologenRoles.Roles role) {
			string sRole = role.ToString();
			bool access = true;
			string synologenComponentName = Synologen.Business.Globals.ComponentName;
			try {
		...
		private void MemberPageInit() {
			...
			_provider = new Spinit.Wpc.Synologen.Data.SqlProvider(_connectionString);
		}
	}
}