...
namespace Spinit.Wpc.Synologen.OPQ.Business.Test
{
	public class TestInit
	{
		...
		{
			string procedurePath = string.Concat (solutionPath, @"Spinit.Wpc.Synologen.OPQ.Database\Schema Objects\Programmability\Stored Procedures");
		...
		private void CreateTriggers (DataContext context, string solutionPath)
		{
			...
			string triggerPath = string.Concat (solutionPath, @"Spinit.Wpc.Synologen.OPQ.Database\Schema Objects\Tables\Triggers");
			ExecuteScripts (triggerPath);
		}

		private void CreateConstraints (DataContext context, string solutionPath)
		{
			string constraintsPath = string.Concat (solutionPath, @"Spinit.Wpc.Synologen.OPQ.Database\Schema Objects\Tables\Constraints");
			ExecuteScripts (constraintsPath);
		}

		private void CreateTestData (DataContext context, string solutionPath)
		{
			string defaultDataPath = string.Concat (solutionPath, @"Spinit.Wpc.Synologen.OPQ.Database\Scripts\Test Data\Business Layer Tests");
			ExecuteScripts (defaultDataPath);
		}

		private void CreateDefaultData (DataContext context, string solutionPath)
		{
			string defaultDataPath = string.Concat (solutionPath, @"Spinit.Wpc.Synologen.OPQ.Database\Scripts\Default Data");
			ExecuteScripts (defaultDataPath);
		}
