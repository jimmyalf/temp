...
namespace Spinit.Wpc.Synologen.OPQ.Presentation
{
	public class OpqPage : Page
	{
		...
		protected override void OnInit (EventArgs e)
		{
			...
			_shopGroupId = GetCurrentShopGroupId ();
		}
		...
		protected int? GetCurrentShopId ()
		{
			...
			if (!string.IsNullOrEmpty (Request.QueryString ["shopId"])) {
				...
				Business.BUtilities util = new Business.BUtilities (_context);
				_shopGroupId = util.GetShopGroupId ((int) shopId);
			}
			return shopId;
		}

		protected int? GetCurrentShopGroupId ()
		{
			int? shopGroupId = null;
			if (!string.IsNullOrEmpty (Request.QueryString ["shopGroupId"])) {
				shopGroupId = int.Parse (Request.QueryString ["shopGroupId"]);
			}
			return shopGroupId;
		}
