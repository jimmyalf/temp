...
namespace Spinit.Wpc.Synologen.OPQ.Site.Wpc.Synologen
{
	public partial class OpqSubPage : OpqControlPage
	{
		...
		private const string Filetag = "<a href=\"{0}\" class=\"{1}\">{2}</a>";
			...
			try {
				if (!Page.IsPostBack) {
					...
					if (node != null) {
						...
						PopulateShopRoutine (node, MemberShopGroupId != null ? null : MemberShopId, MemberShopGroupId);
						PopulateCentralDocuments (node);
						PopulateShopRoutineDocuments (node, MemberShopGroupId != null ? null : MemberShopId, MemberShopGroupId);
						PopulateShopDocuments (node, MemberShopGroupId != null ? null : MemberShopId, MemberShopGroupId);
					}
				}
			}
		...
		private void PopulateShopRoutineDocuments (Node node, int? shopId, int? shopGroupId)
		{
			if (node == null) return;
			if (shopId <= 0) return;
			var bFile = new BFile (_context);
			var files = bFile.GetFiles (node.Id, shopId, null, shopGroupId, FileCategories.ShopRoutineDocuments, true, true, true);
			rptShopRoutineDocuments.DataSource = files;
			rptShopRoutineDocuments.DataBind ();
		}

		private void PopulateShopDocuments (Node node, int? shopId, int? shopGroupId)
		{
			if (node == null) return;
			if (shopId <= 0) return;
			var bFile = new BFile (_context);
			var files = bFile.GetFiles (node.Id, shopId, null, shopGroupId, FileCategories.ShopDocuments, true, true, true);
		...
		private void PopulateCentralDocuments (Node node)
		{
			if (node == null) return;
			var bFile = new BFile (_context);
			var files = bFile.GetFiles (node.Id, null, null, null, FileCategories.SystemRoutineDocuments, true, true, true);
			rptCentralDocuments.DataSource = files;
			rptCentralDocuments.DataBind ();
		}

		private void PopulateShopRoutine (Node node, int? shopId, int? shopGroupId)
		{
			if (node == null) return;
			if (shopId <= 0) return;
			var bDocument = new BDocument (_context);
			var documents = bDocument.GetActiveDocuments (node.Id, shopId, null, shopGroupId, DocumentTypes.Routine, null, false);
		...
		private void PopulateCentralRoutine (Node node)
		{
			if (node == null) return;
			var bDocument = new BDocument (_context);
			var documents = bDocument.GetActiveDocuments (node.Id, null, null, null, DocumentTypes.Routine, null, false);
			if (documents.Count == 0) return;
			Document document = documents [0];
		...
		protected void btnSend_Click (object sender, EventArgs e)
		{
			...
			var document = (MemberShopId > 0)
			               	? bDocument.CreateDocument (
			               		_nodeId,
			               		MemberShopGroupId != null ? null : MemberShopId,
			               		MemberShopGroupId,
			               		DocumentTypes.Proposal,
			               		txtImprovements.Text)
			               	: bDocument.CreateDocument (_nodeId, null, null, DocumentTypes.Proposal, txtImprovements.Text);
			if (document != null) {
