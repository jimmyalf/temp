...
namespace Spinit.Wpc.Synologen.Invoicing{
	public class CommonConversion 
	{
		...
		{
			if(order == null || order.ContractCompany == null || order.ContractCompany.InvoiceFreeTextFormat == null) return new List<string>();
			//if (String.IsNullOrEmpty(company.InvoiceFreeTextFormat)) 
			var parsedInvoiceFreeText = order.ParseFreeText();
				//ParseInvoiceFreeTeext(company.InvoiceFreeTextFormat, order, company);
