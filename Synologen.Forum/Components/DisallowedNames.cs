...
namespace Spinit.Wpc.Forum
{
    ...
    {
      string namesKey = "AspNetForums-DisallowedNames";

      // Serve from the cache when possible
      //
      ArrayList namesCollection = (ArrayList) HttpRuntime.Cache[namesKey];
      if(namesCollection == null) {
        ...
        if(namesCollection != null) {
            ...
            HttpRuntime.Cache.Insert(namesKey, namesCollection, null, DateTime.Now.AddSeconds(1800), TimeSpan.Zero);
        }
      }
        
      return (ArrayList) HttpRuntime.Cache[namesKey];
    }      
    #endregion
